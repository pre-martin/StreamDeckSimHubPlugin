...
Each blink modifier has its own timer that alternates the visibility of the display item. The timer starts the moment the condition becomes true. This means that if there are two or more blink modifiers with different conditions on different display items, the items will be displayed or hidden at different time intervals.

The diagram below shows two different modifiers ("Modifier 1" and "Modifier 2") with their conditions, and how the condition becoming true affects the visibility of the display item.
