...
Each blink modifier has its own timer that alternates the visibility of the display item. The timer starts the moment the condition becomes true. This means that if there are two or more blink modifiers with different conditions on different display items, the items will be displayed or hidden at different time intervals. See section <<_blink_override>> below if synchronization of multiple blink modifiers is desired.
