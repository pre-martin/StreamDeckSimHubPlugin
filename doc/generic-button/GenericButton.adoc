...
Displays a colored box on the button. This can be used to create a background for other display items.

image:Display-Item-Box.png[Display Item Box]

===== Field "Corner Radius"

The corner radius of the box. If this is empty, the box will have sharp corners. If a value is given, the corners will be rounded with the given radius.

The label to the right of the input field shows the maximum possible corner radius for the given size of the box. If the given value is larger than the maximum possible corner radius, it will be clamped to the maximum. If the box has no specific size (like in the screenshot above), the maximum value depends on the size of the Stream Deck key.

The corner radius can also be used to create circles, as long as the size of the box is square (see above).
...
Modifiers allow to change the visual appearance of display items. Each modifier has a condition that determines whether the modifier is active or not.

===== Modifier Blink

Causes the display item to blink (alternating between visible and hidden) as long as the modifier condition is true.

You can read this and the next section, or watch a https://youtu.be/vRb1mFhEyMs[video on YouTube].

Each blink modifier has its own timer that alternates the visibility of the display item. The timer starts the moment the condition becomes true. This means that if there are two or more blink modifiers with different conditions on different display items, the items will be displayed or hidden at different time intervals. See section <<#blinkOverride>> below if synchronization of multiple blink modifiers is desired.

The following diagram shows two different modifiers ("Modifier 1" and "Modifier 2") with their conditions, and how the condition becoming true affects the visibility of the display item:

ifndef::env-github[]
[plantuml, "timing-example", svg]
.Timing diagram for two blink modifiers
----
@startuml
concise "Modifier 1 Condition" as cond1
robust "Modifier 1" as mod1
concise "Modifier 2 Condition" as cond2
robust "Modifier 2" as mod2

mod1 has Visible,Hidden
mod2 has Visible,Hidden
...
ifdef::env-github[]
image:timing-example.svg[Timing diagram for two blink modifiers]
endif::[]

[#blinkOverride]
===== Blink Override

Blink Override allows to synchronize multiple blink modifiers on the same Stream Deck key. If Blink Override is enabled, it will control a single timer for all blink modifiers on this Stream Deck key. This means that all blink modifiers on this key will be visible or hidden at the same time, even if their conditions become true at different times.

Blink Override can be configured in the settings of the Generic Button action. As this is a setting for the whole action, it can be found in the top right corner of the editor (button with gear icon).

The diagram below shows this single timer in the first row, and how the blink modifiers only participate when their condition is true. The items are visible and hidden synchronously, as long as the blink condition is true:

ifndef::env-github[]
[plantuml, "timing-example-with-override", svg]
.Timing diagram for two blink modifiers with Blink Override
----
@startuml
robust "Blink Override" as override
concise "Modifier 1 Condition" as cond1
robust "Modifier 1" as mod1
concise "Modifier 2 Condition" as cond2
robust "Modifier 2" as mod2

override has Visible,Hidden
mod1 has Visible,Hidden
mod2 has Visible,Hidden
...
@2
cond1 is "True"
cond1 -> mod1 : Participate in Override
mod1 is Visible
@4
mod1 is Hidden
@6
...
@11
cond1 is "False"
cond1 -> mod1: Stop Participate in Override
mod1 is Visible
@12
...
@3
cond2 is "True"
cond2 -> mod2 : Participate in Override
mod2 is Visible
@4
mod2 is Hidden
@6
...
@11
cond2 is "False"
cond2 -> mod2: Stop Participate in Override
mod2 is Visible
...
ifdef::env-github[]
image:timing-example-with-override.svg[Timing diagram for two blink modifiers with Blink Override]
endif::[]


===== Modifier Color

Changes the color of items that have a "Color" field.
...
Profiles containing Generic Button actions can be exported and imported like any other Stream Deck profile, via Settings -> Profiles -> Export or by double-clicking the "streamDeckProfile" file (import).

However, there is one special aspect to note: *The images used are not exported or imported* (because the Generic Button manages the images itself instead of using the limited functionality of the Stream Deck software). Therefore, the images themselves must be copied manually from or to `%APPDATA%\Elgato\StreamDeck\Plugins\net.planetrenner.simhub.sdPlugin\images\custom`.

When importing into a Stream Deck device with a different resolution (e.g. importing from a standard-res Stream Deck to a high-res Stream Deck), the plugin automatically scales the display elements to the size of the target device.

== Built-in Properties

In addition to SimHub properties, the Generic Button has access to built-in properties. These properties are produced locally by the plugin itself (no SimHub connection required) and can be used in conditions, expressions, and display values exactly like regular SimHub properties.

All built-in properties use the prefix `StreamDeckSimHub.`.

[cols="1,1,3"]
|===
|Property |Type |Description

|`StreamDeckSimHub.Connection.Connected` | Boolean (`true` / `false`) | Indicates whether the plugin is currently connected to the SimHub Property Server.

|`StreamDeckSimHub.Subscriptions.Count` | Integer | The number of SimHub properties currently subscribed to by the plugin.
|===